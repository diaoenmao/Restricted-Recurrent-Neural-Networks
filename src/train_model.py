...
def main():
    ...
    for i in range(config.PARAM['num_Experiments']):
        model_TAG = '{}_{}_{}_{}'.format(seeds[i], config.PARAM['data_name']['train'], config.PARAM['model_name'],
            ...
            if (config.PARAM['special_TAG'] == '') else '{}_{}_{}_{}_{}'.format(seeds[i],
...
def runExperiment(model_TAG):
    ...
    data_loader = split_dataset(dataset, data_size=config.PARAM['data_size'], batch_size=config.PARAM['batch_size'],
                                shuffle={'train': False, 'test': False}, radomGen=config.PARAM['randomGen'])
    print(config.PARAM)
    print('Training data size {}, Number of Batches {}'.format(config.PARAM['data_size']['train'],
    ...
    if (config.PARAM['resume_mode'] == 1):
        last_epoch, model, optimizer, scheduler, train_meter_panel, test_meter_panel = resume(model, optimizer,
                                                                                              scheduler, model_TAG)
    ...
    cur_test_meter_panel = test(data_loader['test'], model, -1, model_TAG)
    print(cur_test_meter_panel.summary(['loss'] + config.PARAM['metric_names']['test']))
    for epoch in range(last_epoch, config.PARAM['num_epochs'] + 1):
        ...
        if (config.PARAM['save_mode'] >= 0):
            ...
            save_result = {'config': config.PARAM, 'epoch': epoch + 1, 'model_dict': model_state_dict,
                           ...
                           'scheduler_dict': scheduler.state_dict(), 'train_meter_panel': train_meter_panel,
                           'test_meter_panel': test_meter_panel}
...
def train(train_loader, model, optimizer, epoch, model_TAG):
    ...
    for i, input in enumerate(train_loader):
        ...
        meter_panel.update(evaluation, len(input['bits']))
        ...
        if (i % (len(train_loader) // 5) == 0):
            ...
            print('Train Epoch({}): {}[({:.0f}%)]{}, Estimated Finish Time: {}'.format(
                model_TAG, epoch, 100. * i / len(train_loader),
...
def make_scheduler(optimizer):
    if (config.PARAM['scheduler_name'] == 'MultiStepLR'):
        ...
        scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=config.PARAM['factor'], verbose=True,
                                      threshold=config.PARAM['threshold'],
                                      threshold_mode=config.PARAM['threshold_mode'])
...
def print_result(model_TAG, epoch, train_meter_panel, test_meter_panel, lr):
    estimated_finish_time = str(
        datetime.timedelta(seconds=(config.PARAM['num_epochs'] - epoch) * train_meter_panel.panel['batch_time'].sum))
    print('***Test Epoch({}): {}{}, Estimated Finish Time: {}, Learnint Rate: {}'.format(model_TAG, epoch,
                                                                                         test_meter_panel.summary(
                                                                                             ['loss'] + config.PARAM[
                                                                                                 'metric_names'][
                                                                                                 'test']),
                                                                                         estimated_finish_time, lr))
    return
