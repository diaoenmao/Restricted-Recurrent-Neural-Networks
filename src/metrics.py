    ...
    def eval(self, input, output, metric_names):
        ...
        if ('classification' in config.PARAM['tuning_param'] and config.PARAM['tuning_param']['classification'] > 0):
            ...
            if ('cluster_acc' in metric_names):
                evaluation['cluster_acc'] = Cluster_ACC(self.score, self.label, topk=topk) if (
                config.PARAM['activate_full']) else 0
            if ('roc' in metric_names):
                evaluation['roc'] = ROC(self.score, self.label) if (config.PARAM['activate_full']) else {'fpr': 0,
                                                                                                         'tpr': 0,
                                                                                                         'roc_auc': 0}
        return evaluation
