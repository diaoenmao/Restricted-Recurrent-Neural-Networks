...
def main():
    control_names = [['0'],[config.PARAM['data_name']['train']],['pixelshufflerescodec32'],['1'],['8'],['2','4','8','16','32'],['2']]
...
def gather_result(model_TAGs):
    ...
    for i in range(len(model_TAGs)):
        ...
        if(os.path.exists(result_path)):
            ...
            'BPP':result['test_meter_panel'].panel['bpp'].history_avg[-1],
            'loss':result['test_meter_panel'].panel['loss'].history_avg[-1],
            'PSNR':result['test_meter_panel'].panel['psnr'].history_avg[-1]}
        else:
...
def process_result(control_names):
    ...
    result = load(result_path)
    evaluation_names = ['BPP','loss','PSNR']
    processed_result = {}
